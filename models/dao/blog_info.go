...
type BlogInfo struct {
	...
	BlogCreateTime     string `orm:"auto_now_add;type(datetime);description(文章创建时间)"` //auto_now_add 第一次保存时才设置时间   date 是日期
	...
	BlogLastModifyTime string `orm:"auto_now;type(datetime);description(文章最后一次修改时间)"` //auto_now 每次 model 保存时都会对时间自动更新
